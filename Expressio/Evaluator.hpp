 ...
 * @endcode
 *
 * All builtin integer types (`int`, `short`, ...), floating point types (`float`, `double`, ...) and complex types (`std::complex<double>`, ...)
 * can naturally be used to instantiate `Evaluator`, as they provide the required interface.
 */
...
private:
	...
	struct Operator : public Function
	{
		...
		{
			left, ///< If the operator is left associative, operators of equal precedence will be evaluated from left to right
			right ///< If the operator is right associative, operators of equal precedence will be evaluated from right to left
		};
		
		Associativity associativity; ///< The associativity of the operator
...
template<class ValueType>
void Evaluator<ValueType>::addFunction(const std::string& funcName, const Function& function)
{
	functionMap.emplace(funcName, function);
}

template<class ValueType>
ValueType Evaluator<ValueType>::evaluate(const std::string& expression) const
{
...
template<class ValueType>
template<class CharacterPredicate>
std::string Evaluator<ValueType>::nextPredicatedString(const std::string& s, CharacterPredicate func)
{
...
template<class ValueType>
typename Evaluator<ValueType>::Operator Evaluator<ValueType>::getOperator(const std::string& operatorSymbol)
{
...
template<class ValueType>
typename Evaluator<ValueType>::Function Evaluator<ValueType>::getFunction(const std::string& functionName) const
{
...
template<class ValueType>
std::string Evaluator<ValueType>::handleLeadingUnaryNegation(const std::string& expression) const
{
...
template<class ValueType>
std::vector<typename Evaluator<ValueType>::InfixToken> Evaluator<ValueType>::tokenize(std::string expression) const
{
...
template<class ValueType>
std::vector<typename Evaluator<ValueType>::PostfixToken> Evaluator<ValueType>::infixToPostfix(const std::vector<InfixToken>& infixTokens) const
{
...
template<class ValueType>
ValueType Evaluator<ValueType>::evaluatePostfix(const std::vector<PostfixToken>& postfixTokens) const
{
